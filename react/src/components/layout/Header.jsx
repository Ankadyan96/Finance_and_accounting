...
// Defensive fallback for the avatar badge — it should always show a short
// 1-3 character abbreviation. `initials` normally already is one, but if
// it's ever missing or (as happened once, transiently, from a bad rename
// sweep) holds something longer like a full entity name, derive one from
// the initial letters of `name` instead of overflowing the small circular
// badge with wrapped text.
function safeInitials(initials, name) {
  if (initials && initials.length <= 3) return initials;
  if (name) {
    const letters = name.match(/[A-Za-z0-9]/g) || [];
    if (letters.length > 0) return letters.slice(0, 2).join('').toUpperCase();
  }
  return initials ? initials.slice(0, 3) : 'US';
}
    ...
    <header className="app-header" role="banner">
      ...
      <div className="header-actions">
        ...
        <div className="v-user-profile-wrap" ref={userRef}>
          ...
          >
            {safeInitials(currentUser?.initials, currentUser?.name) || 'JB'}
          </button>
