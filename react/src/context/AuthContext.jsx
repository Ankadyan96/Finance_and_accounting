...
export function AuthProvider({ children }) {
  ...
  const [currentUser, setCurrentUser] = useState(() => {
    ...
    return MOCK_USERS[0]; // Default to Vikram & Sons Carrier
  });
  ...
  useEffect(() => {
    api.getUsers().then(dbUsers => {
      if (Array.isArray(dbUsers) && dbUsers.length > 0) {
        ...
        // Re-sync currentUser's own display fields against this fresh data.
        // currentUser is initialized straight from localStorage/sessionStorage
        // (see above) and nothing else ever refreshes it — so a demo
        // account whose name/entity/initials changed server-side (e.g. a
        // renamed entity) would otherwise keep showing the stale cached
        // values in the header/avatar forever, until an explicit re-login.
        setCurrentUser(prev => {
          if (!prev?.email) return prev;
          const fresh = mapped.find(u => u.email === prev.email);
          if (!fresh) return prev;
          const isStale = fresh.name !== prev.name
            || fresh.entityName !== prev.entityName
            || fresh.initials !== prev.initials
            || fresh.businessLabel !== prev.businessLabel
            || fresh.roleLabel !== prev.roleLabel
            || fresh.avatarColor !== prev.avatarColor;
          if (!isStale) return prev;
          const next = { ...prev, ...fresh };
          try {
            const storage = localStorage.getItem('v_user') ? localStorage : sessionStorage;
            storage.setItem('v_user', JSON.stringify(next));
          } catch {
  ...
  const activeEntity = useMemo(() => {
    ...
    const fallbackName = isPizza ? "Domino's Main Company" : 'Vikram & Sons';
