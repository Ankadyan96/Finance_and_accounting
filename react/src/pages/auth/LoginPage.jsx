...
export function LoginPage() {
  ...
  // Set from the Configuration Centre's "Login Screen Options" toggle
  // (AdminConfigPage.jsx) — read directly from localStorage since this
  // page renders before anyone is authenticated.
  const [showPizzaOption] = useState(() => {
    try {
  ...
  const [selectedLevel, setSelectedLevel] = useState(() => {
    try {
      const saved = sessionStorage.getItem('v_accounting_level') || localStorage.getItem('v_accounting_level') || 'insurance';
      // A saved 'pizza' selection from before the toggle was switched off
      // shouldn't strand the login screen on a level it no longer offers.
      if (saved === 'pizza' && localStorage.getItem('v_show_pizza_login') !== 'true') return 'insurance';
      return saved;
    } catch {
      return 'insurance';
    }
    ...
    <div className="auth-wrap">
      ...
      <div className="auth-panel">
        ...
        <div className="auth-right">
          ...
          {/* TWO BUSINESS / ACCOUNTING LEVEL OPTIONS — only shown when the
              Configuration Centre's "Login Screen Options" toggle has
              Domino's Pizza enabled; otherwise Insurance is the only level
              anyway, so the picker is skipped entirely. */}
          {showPizzaOption && (
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: isActive ? '0 2px 8px rgba(249, 115, 22, 0.35)' : 'none',
                      borderRadius: '7px',
                      padding: '5px 10px',
                      fontSize: '11px',
                      fontWeight: 600,
