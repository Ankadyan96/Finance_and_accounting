        ...
        <div className="tab-panel active">
          ...
          <div className="filter-bar" id="je-dimension-filter-bar" style={{ marginTop: 0, marginBottom: '16px', flexWrap: 'wrap', gap: '8px' }}>
            ...
            <select className="filter-select" value={filterBroker} onChange={e => setFilterBroker(e.target.value)}>
              ...
              <option value="Arora & Sons">Arora & Sons</option>
            ...
            <select className="filter-select" value={filterCarrier} onChange={e => setFilterCarrier(e.target.value)}>
              ...
              <option value="Vikram & Sons">Vikram & Sons</option>
            </select>
                ...
                <tbody>
                  {(selectedEntry.lines || []).map((l, i) => {
                    ...
                    if (isPizzaMode) {
                      ...
                      // Trust the line's own accountName when it's set — some
                      // accounts (e.g. 2200 "Net Premium Payable") are reused
                      // for more than one counterparty across a chain (Broker
                      // → MGA vs MGA → Carrier), and the Chart of Accounts
                      // only carries one fixed name for the code. Lines that
                      // don't set their own name already resolve to the COA
                      // name at creation time, so this only changes display
                      // for the lines that deliberately override it.
                      lineName = lineName || (matchedAcct ? matchedAcct.name : '—');
                    }
